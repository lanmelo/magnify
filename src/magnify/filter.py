...
def filter_leaky_buttons(assay: xr.Dataset, search_channel: str | list[str] | None = None):
    ...
    # Vertical (up/down) neighbors are num_cols apart in the flattened mark order.
    num_cols = int(assay.mark_col.max()) + 1
    for channel in search_channels:
        ...
        for i in range(assay.sizes["mark"]):
            ...
            if tag[i] == "":
                continue
            if row > 0 and tag[i - num_cols] == "":
                valid[i] &= empty[i - num_cols]
            if row < assay.mark_row.max() and tag[i + num_cols] == "":
                valid[i] &= empty[i + num_cols]
    assay.valid.data = valid
